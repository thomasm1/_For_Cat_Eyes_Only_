...
class Login extends React.Component {
  ...
  componentWillMount() {
    !!(this.props.isLoggedIn) && browserHistory.push('/producer');
  }
  componentWillReceiveProps(nextProps) {
    !!(nextProps.isLoggedIn) && browserHistory.push('/producer');
  }
  render() {
    return (
      <div className={styles.Login}>
        <form onSubmit={(e) => {
          ...
          <div className={'row'}>
            <p className={'text-center'}><a className="brand-logo"><img src={require('./images/gasco.png')}/> GasCo</a></p>
          ...
          <p className="text-center">
            <small>{`Don't have an account? Create one `}<Link to={'/producer/signup'}>{'here'}</Link></small>
          </p>
        </form>
      </div>
    );
